...
char* appName = "Marsens";
char* windowText = "Marsens";
int windowSizeX = 800;
int windowSizeY = 600;
...
void initUi(LPDIRECT3DDEVICE9 dxDevice)
{
	...
	temp = new uiItem(dxDevice, "leftover", NULL, UIT_button, vertexDecPCT, "un_shade.fx", "over_final", "over_left", rect, &effects, &textures);
	...
	rect.top = 0;
	rect.bottom = winHeight;
	temp = new uiItem(dxDevice, "rightover", NULL, UIT_button, vertexDecPCT, "un_shade.fx", "over_final", "over_right", rect, &effects, &textures);
	temp->enabled = true; // NEED to work out why these shaders are so whiney (simpleUi uses linear sampler, can't do linear sample on render target?)
	temp->clickable = false;
	uiItems.push_back(temp);
	temp->colMod = D3DXVECTOR4(0, 1, 1, 1);
...
void initObjs(LPDIRECT3DDEVICE9 dxDevice)
{
	...
	UNCRZ_obj* temp;
	temp = new UNCRZ_obj(new UNCRZ_model(mapObj->model));
	temp->changeAnim(mapAnim);
	temp->offset = D3DXVECTOR3(-100, 0, 0);
	...
	temp = new UNCRZ_obj(new UNCRZ_model(mapObj->model));
	temp->changeAnim(mapAnim);
	temp->offset = D3DXVECTOR3(0, 0, -100);
	zSortedObjs.push_back(temp);
	objs.push_back(temp);
...
void initViews(LPDIRECT3DDEVICE9 dxDevice)
{
	...
	tempView = new UNCRZ_view("left");
	tempView->camPos = D3DXVECTOR3(-30, 20, -30);
	tempView->camPos += D3DXVECTOR3(0.5, 0, -0.5); // stereo??
...
void initOvers(LPDIRECT3DDEVICE9 dxDevice)
{
	...
	tempOver = new UNCRZ_over("left");
	tempOver->init(dxDevice, vpWidth * targetTexScale, vpHeight * targetTexScale, "un_shade.fx", "over_greys", &effects, D3DFMT_A8B8G8R8);
	tempOver->initStencil(dxDevice, zSurface);
	tempOver->useTex = true; // write a "loadtex" func or something?
	createTexture(dxDevice, "view_left", &tempOver->tex, &textures);
	...
	tempOver = new UNCRZ_over("right");
	tempOver->init(dxDevice, vpWidth * targetTexScale, vpHeight * targetTexScale, "un_shade.fx", "over_greys", &effects, D3DFMT_A8B8G8R8);
	tempOver->initStencil(dxDevice, zSurface);
	tempOver->useTex = true; // write a "loadtex" func or something?
	createTexture(dxDevice, "view_right", &tempOver->tex, &textures);
	tempOver->alphaMode = AM_none;
	tempOver->clearView = true;
	tempOver->clearColor = D3DCOLOR_XRGB(0, 0,	 0);
	tempOver->colMod = D3DXVECTOR4(1, 1, 1, 1);
